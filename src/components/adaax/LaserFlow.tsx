...
/**
 * LaserFlow — a WebGL volumetric laser beam (three.js raw shader).
 *
 * Ported from the "Laser Focus" reference and recoloured to the AdaaX brand
 * green. This is the ONLY client-side WebGL on the site and lives solely on the
 * /adaax landing hero. All GL/browser access happens inside useEffect, so the
 * component server-renders to an empty <div> with no hydration cost.
 *
 * The beam descends from the top and "lands" on the AdaaX dashboard below it.
 */
...
const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  falloffStart = 1.2,
  fogFallSpeed = 0.6,
  // AdaaX brand-green default (was red in the reference)
  color = "#3ee07a",
}) => {
  ...
  useEffect(() => {
    ...
    // Gracefully degrade when WebGL is unavailable (old GPUs, disabled, headless):
    // the beam simply doesn't render, the rest of the hero is unaffected.
    ...
    // Transparent clear so the beam composites over the hero's dark backdrop.
